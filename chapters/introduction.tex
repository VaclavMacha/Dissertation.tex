...
\begin{mini}{\bm{w}, t}{
    \lambda_1 \sum_{i \in \Ineg} \Iverson{s_i \ge t} + \lambda_2 \sum_{i \in \Ipos} \Iverson{s_i < t}
...
\begin{figure}
  ...
  \begin{NiceTabular}{cccccc}[cell-space-limits = 7pt]
    ...
    & $\rightarrow$
    & \Block[draw=black, rounded-corners]{1-1}{\longcell{all \\ negatives \\ ($\nneg$)}} \\
    & $y = 1$
    & \Block[draw=myred, fill=myred!50, rounded-corners]{1-1}{
    ...
    & $\rightarrow$
    & \Block[draw=black, rounded-corners]{1-1}{\longcell{all \\ positives \\ ($\npos$)}} \\
    && $\downarrow$
    &  $\downarrow$ \\
