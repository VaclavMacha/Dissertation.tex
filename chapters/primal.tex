...
The decision variables in \eqref{eq: aatp surrogate} are the normal vector of the separating hyperplane~$\bm{w}$ and the threshold~$t$. Since the gradient descent algorithm uses the following rule
\begin{equation*}
...
\begin{restatable}{theorem}{sgd}\label{thm:sgd}
  Consider the \PatMat or \PatMatNP formulation, stepsizes
  \begin{equation*}
    \alpha^k = \frac{\alpha^0}{k+1}
  \end{equation*}
  and piecewise disjoint minibatches~$\Imb^1, \; \Imb^2, \ldots, \; \Imb^m$ which cycle periodically~$\Imb^{k+m} = \Imb^k$. If~$l$ is the smoothened hinge function defined by
