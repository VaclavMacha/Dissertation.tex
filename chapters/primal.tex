...
In this chapter, we focus on the special case when the classifier is linear
\begin{equation}
...
\end{itemize}
For a nicer flow of text, we show the results only for formulations from Section~\ref{sec: aatp}. The results for methods from Section~\ref{sec: Neyman-Pearson} are identical. For the same reason, we postpone the proofs to Appendix~\ref{chap: linear}.
...
\begin{restatable}{theorem}{derivative}\label{thm:derivative}
  If the surrogate function~$l_{\vartheta}$ is differentiable, then threshold~$t_3$ is a differentiable function of the weights~$\bm{w}$ and its derivative equals to
  \begin{equation*}
    \nabla t_3(\bm{w}) = \frac{\sum_{i \in \I} l_{\vartheta}'(\bm{w}^\top \bm{x}_i - t_3(\bm{w}))\bm{x}_i}{\sum_{i \in \I}l_{\vartheta}'(\bm{w}^\top \bm{x}_i - t_3(\bm{w}))}.
  \end{equation*}
...
\begin{figure}
  ...
  \caption{Left: distribution of positive (red circles) and negative samples (blue circles) for the example from Section~\ref{sec:example}. Right: contour plot of the objective function value for \TopPush and its convergence (orange lines) to the zero vector from~$12$ initial points.}
  \label{fig:example}
\end{figure}

Table~\ref{tab:example} shows the threshold~$t$ and the objective value~$L$ for two points~$\bm{w}_1=(0,0)$ and~$\bm{w}_2=(1,0)$. These two points are both important:~$\bm{w}_1$ does not generate any separating hyperplane, while~$\bm{w}_2$ generates the optimal separating hyperplane. We show the precise computation in Appendix~\ref{app:example}. Since the dataset is perfectly separable by~$\bm{w}_2$, we expect that~$\bm{w}_2$ provides a lower objective than~$\bm{w}_1$. By highlithing the better objective in Table~\ref{tab:example} by green, we see that this did not happen for \TopPush and \TopMeanK.
...
\begin{table}[!ht]
  ...
  \begin{NiceTabular}{lccccc}
    ...
    \Block{2-1}{\textbf{Name}}
      ...
      & \Block{1-2}{$\bm{w}_1=(0,0)$}
      && \Block{1-2}{$\bm{w}_2=(0,0)$} \\
    ...
    \bottomrule
  \end{NiceTabular}
  \caption{Comparison of formulations on the very simple problem from Section~\ref{sec:example}. Two formulations have the global minimum (denoted by green color) at~$\bm{w}_1=(0,0)$ which does not generate any separating hyperplane. The optimal separating hyperplane is generated by~$\bm{w}_2=(1,0)$.}
  \label{tab:example}
\end{table}
...
\begin{restatable}{theorem}{larget}\label{thm:large_t}
  ...
  \begin{equation}\label{eq:w_zero}
    ...
    \frac{1}{K} \sum_{i=1}^{K} s_{[i]} \le \sum_{i=1}^{n^+} s_{i}^+
...
\begin{restatable}{theorem}{patmatzero}\label{thm:patmat_zero}
  Consider the \PatMat or \PatMatNP formulation with the hinge surrogate with scaling parameter~$\vartheta$ and no regularization. Assume that for some~$\bm{w}$ we have
  \begin{equation}\label{eq:patmat_zero}
    \frac{1}{n^+}\sum_{i \in \I^+}\bm{w}^\top \bm{x}_i > \frac{1}{n^-}\sum_{j \in \I^-}\bm{w}^\top \bm{x}_j.
  \end{equation}
  Then there is a scaling parameter~$\vartheta_0$ from~\eqref{eq: aatp quantile surrogate} such that~$L(\bm{w}) < L(\bm{0})$ for all~$\vartheta \in (0, \vartheta_0)$.
\end{restatable}
...
\begin{table}[!ht]
  ...
  \begin{tabular}{ll ccccc}\toprule
    ...
    \tauFPL &~\cite{zhang2018tau} & \eqref{eq: tau-fpl} &~$\lambda$ & \yesmark & \nomark & \nomark\\
    \bottomrule
  \end{tabular}
