...
The second problem from Section~\ref{sec: related problems} is the problem of Accuracy at the Top~\cite{boyd2012accuracy}. This problem aims to find an ordering of samples so that samples whose scores are among the top $\tau$-quantile are as relevant as possible. In statistics, the standard definition of the top~$\tau$-quantile of all scores is following
...
Both \TopMeanK and \PatMat use a surrogate approximation of false-negative rate as an objective function. Since this approximation is a non-decreasing function of~$t,$ a lower threshold~$t$ means a lower value of the objective function. Therefore, a lower threshold is preferred. It seems that \TopMeanK is a better formulation since it uses a better approximation of the true quantile. Besides that, it is also easier to compute the threshold for \TopMeanK than for \PatMat. However, Chapter~\ref{chap: linear} shows that \PatMat formulation has better properties than \TopMeanK. Moreover, in the following section, we show how to efficiently compute the threshold for \PatMat with hinge loss as surrogate function.

\subsection{Efficient Computing of the Threshold for \PatMat}\label{sec: patmat threshold alg}

As we mentioned before, due to the properties of the surrogate function~$l$ (see
Notation~\ref{not: surrogates}), the threshold~\eqref{eq: aatp quantile surrogate} for \PatMat is unique. In this section, we show how to efficiently find the threshold when the hinge loss is used as a surrogate. Consider function
\begin{equation*}
  h(t) = \sum_{i \in \I} l\Brac{\vartheta(s_i - t)} - n\tau.
\end{equation*}
Then finding threshold~\eqref{eq: aatp quantile surrogate} for \PatMat is equivalent to looking for~$\hat{t}$ such that~$h(\hat{t}) = 0$. Function~$h$ is continuous and strictly decreasing until it hits the global minimum. Moreover,~$h(t) \to \infty$ as~$t \to -\infty$ and~$h(t) \to -n\tau$ as~$t \to \infty$. Thus, there is a unique solution to the equation~$h(t) = 0$. For sorted data, the following lemma advises how to solve this equation. The whole procedure of finding~$\hat{t}$ is summarized in Algorithm~\ref{alg: patmat threshold}.
...
\begin{restatable}{lemma}{lemmapatmatalg}\label{lemma: patmat threshold algorithm}
  ...
  \begin{equation}\label{eq:update_h}
    h(t_i)
      = h(t_{i - 1}) + (i - 1) \vartheta(s_{[i - 1]} - s_{[i]}),
  \end{equation}
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \State $t_1 \gets s_{[1]}  + \frac{1}{\vartheta}$
    \State $h(t_1) \gets -n\tau$
    \State $i \gets 2$
    \While{$h(t_i) \geq 0$}
      ...
      \State $i \gets i + 1$
    \EndWhile
    \State \textbf{return} linear interpolation of the last two values~$\hat{t} \gets t_{i-1} + \frac{1}{\vartheta(i-1)}h(t_{i-1})$
  \end{algorithmic}
...
The previous section introduces three formulations \GrillNP, \tauFPL, and \PatMatNP. While \GrillNP formulations use true top $\tau$-quantile~\eqref{eq: NP quantile} as a threshold, the remaining two formulations use only its approximations. The following lemma shows that \tauFPL uses a better approximation of the top $\tau$-quantile than \PatMatNP.
...
Both \tauFPL and \PatMatNP use a surrogate approximation of false-negative rate as an objective function. Since this approximation is a non-decreasing function of~$t,$ a lower threshold~$t$ means a lower value of the objective function. Therefore, a lower threshold is preferred.
...
Since the goal of the presented formulations is to push~$s^+$ above~$s^-$, we may expect that the conditions in Lemma~\ref{lemma: threshold comparison 2} are satisfied. 
