...
\begin{restatable}[Update rule for~$\Delta^{\star}$ for \TopPushK with truncated quadratic loss]{theorem}{toppushkupdatequadratic}\label{thm:Update rule TopPushK with quadratic loss}
  ...
  \begin{itemize}
    ...
    \end{align*}
    \item For any~$\npos + 1\leq k,l \leq n$ we define~$\hat{k} = k - \npos,$~$\hat{l} = l - \npos$ and then  have
    \begin{align*}
      \Delta_{lb} & = 
        \begin{cases*}
          -\betak & K = 1, \\
