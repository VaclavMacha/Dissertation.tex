...
\end{mini}
where~$C \in \R.$ For \PatMat we have~$\Itil = \I$ and~$\ntil = \nall.$ For \PatMatNP we have~$\Itil = \Ineg$ and~$\ntil = \nneg.$ Again, we use the alternative formulation with constant~$C$ and regularization parameter~$\lambda = 1.$ The following theorem shows the dual form of the formulation~\eqref{eq: patmat family}.
